...
cd $TEMP_DIR
git clone -b $BITBUCKET_PR_DESTINATION_BRANCH https://x-token-auth:$SNOWMATE_ACCESS_TOKEN@bitbucket.org/$BITBUCKET_WORKSPACE/$BITBUCKET_REPO_SLUG.git
...
snowmate_runner run --debug-mode --project-id $PROJECT_ID --client-id $SNOWMATE_CLIENT_ID --secret-key $SNOWMATE_SECRET_KEY --workflow-run-id $BITBUCKET_BUILD_NUMBER --cloned-repo-dir $BASELINE_PROJECT_PATH --project-root-path $FEATURE_PROJECT_PATH --details-url $SNOWMATE_APP_URL/regressions/$PROJECT_ID/$BITBUCKET_BUILD_NUMBER --pull-request-number $BITBUCKET_PR_ID --api-url $SNOWMATE_API_URL --auth-url $SNOWMATE_AUTH_URL --pypi-url $SNOWMATE_PYPI_URL; snowmate_runner_status=$?
...
if [ -n "$SNOWMATE_ACCESS_TOKEN" ]; then
    if [ -e $SNOWMATE_REPORT_FILE_PATH ]; then
        ...
        curl --request POST --url $PULL_REQUESTS_COMMENTS_API_URL --header "Content-Type: application/json" --header "Authorization: Bearer $SNOWMATE_ACCESS_TOKEN" --data "{ \"content\": { \"raw\": $markdown_content } }" > /dev/null
    else
